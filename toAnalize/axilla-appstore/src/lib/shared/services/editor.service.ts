...
export class JsonEditorService {
  ...
  mode = abp.setting.get("App.VDB.JsonView.DefaultView") as any;

  private _initEditoActions(editorOptions: JsonEditorOptions) : JsonEditorOptions {
    editorOptions.onCreateMenu = function (items, node) {
      ...
      const listOfActions: { [key: string]: { [key: string]: string } }[] = [
    ...
    editorOptions.onModeChange = function (newMode, oldMode) {
    ...
    editorOptions.onChangeText = function (jsonstr) {
      try {
  ...
  initEditor() : JsonEditorOptions {
    let editorOptions = new JsonEditorOptions();

    editorOptions.modes = ['tree', 'code'];
    editorOptions.mode = this.mode;

    // show top toolbar / breadcrumbs / status bar (optional but useful)
    editorOptions.mainMenuBar = true;
    editorOptions.navigationBar = true;
    editorOptions.statusBar = true;

    editorOptions = this._initEditoActions(editorOptions);
