...
import {
    AxillaCommonModule, AppSessionService, DomHelper, CookieConsentService
    , AppUiCustomizationService, UrlHelper, LocaleMappingService, IApplicationInfoDto
    , IUiCustomizationSettingsDto
...
import { AppModule } from '@root/app';
import { RootRoutingModule } from './root-routing.module';
import { RootComponent } from './root.component';
import { NgxBootstrapDatePickerConfigService } from '@axilla/axilla-shared';
...
function initializeLocalForage() {
    localForage.config({
        ...
        description: 'Cached data for Ublion'
    });
}
...
function setApplicationInfoForInstallPage(injector, theme: UiCustomizationSettingsDto) {
    ...
    appSessionService.application = {} as IApplicationInfoDto;
    appSessionService.application.releaseDate = dayjs(dayjs().startOf('day').toISOString());
}
...
function initializeAppCssClasses(injector: Injector, theme: IUiCustomizationSettingsDto) {
    let appUiCustomizationService = injector.get(AppUiCustomizationService);
    appUiCustomizationService.init(theme as UiCustomizationSettingsDto);
...
function registerLocales(resolve: (value?: boolean | Promise<boolean>) => void, reject: any, spinnerService: NgxSpinnerService) {
    if (shouldLoadLocale()) {
        ...
        import ('../node_modules/@angular/common/locales/global/nl')  .then(module => {
