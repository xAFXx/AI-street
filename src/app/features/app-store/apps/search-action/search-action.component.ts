...
import { AgentMentionEditorComponent, MentionMessage } from '../../../../shared/components/agent-mention-input/agent-mention-input.component';
import { AiChatHubService } from '../../../../core/services/ai-chat-hub.service';
import { ChatConnectionState } from '../../../../core/models/ai-chat.models';
import { AIAgentDto } from '../../../ai-management/api/ai-management.models';
...
export class SearchActionComponent implements OnInit, OnDestroy {
    ...
    private chatHub = inject(AiChatHubService);
    ...
    // Agent mention
    selectedAgent: AIAgentDto | null = null;

    onAgentSelected(agent: AIAgentDto): void {
        this.selectedAgent = agent;
        console.log('[Search & Action] Agent selected:', agent.name);
    }

    onMentionMessage(event: MentionMessage): void {
        this.searchQuery = event.text;
        this.selectedAgent = event.agent;
        console.log('[Search & Action] Message:', event.text, 'Agent:', event.agent?.name);
        this.performSearch();
    }
    ...
    hubConnectionState = computed(() => this.chatHub.connectionState());
    isHubConnected = computed(() => this.chatHub.isConnected());
    ...
    /** Initialize SignalR connection to AI chat hub */
    private async initializeSignalR(): Promise<void> {
        try {
            await this.chatHub.initialize();
            console.log('[Search & Action] SignalR connected — state:', this.hubConnectionState());
        } catch (err) {
            console.warn('[Search & Action] SignalR connection failed:', err);
        }
    }
