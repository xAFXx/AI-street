...
export class DocumentManagementComponent implements OnInit, OnDestroy {
    ...
    selectedSchema = computed(() => {
        ...
        return id ? this.schemaService.getSchemaById(id) ?? null : null;
    });
        ...
        const processFile = async (fileIndex: number): Promise<void> => {
            ...
            try {
                ...
                if (needsVisionApi && file.file) {
                    // Use Vision API for PDFs and images - combined analysis and schema mapping
                    // This allows the AI to see the actual document and extract structured data in one step
                    const schema = this.selectedSchema();
                    const visionMappingPrompt = this.buildVisionMappingPrompt(file, schema);
                    console.log('[DocumentManagement] Using Vision API with schema mapping for:', file.name);
                    result = await this.aiChatService.analyzeFileWithVision(file.file, visionMappingPrompt);
                } else {
                ...
                if (schema) {
                    try {
                        ...
                        // For Vision files, the result already contains schema-mapped JSON
                        // For text files, we need to do the mapping step
                        if (needsVisionApi && file.file) {
                            // Result already contains the JSON from Vision API
                            mappingResult = result;
                        } else {
                            // Text files need separate mapping
    ...
    /**
     * Build a Vision API prompt that combines document analysis and schema-based extraction
     * This allows the AI to see the document and extract structured JSON in one step
     */
    private buildVisionMappingPrompt(file: UploadedFile, schema: Schema | null): string {
        if (!schema) {
            // Fallback to regular analysis if no schema
            return this.buildVisionAnalysisPrompt(file);
        }

        // Get the schema JSON for the prompt
        const schemaJson = this.getSchemaJsonForPrompt(schema);

        let prompt = `You are analyzing a document to extract structured data. Your output MUST be a valid JSON object.\n\n`;

        prompt += `=== TARGET JSON SCHEMA ===\n`;
        prompt += `${schemaJson}\n\n`;

        prompt += `=== EXTRACTION RULES ===\n`;
        prompt += `1. Carefully examine ALL pages of the document shown in the images\n`;
        prompt += `2. Extract values for EVERY field in the schema - look thoroughly\n`;
        prompt += `3. Your output MUST be a valid JSON object matching the schema structure EXACTLY\n`;
        prompt += `4. Use the EXACT property names from the schema (case-sensitive)\n`;
        prompt += `5. Numbers must be numeric values (e.g., 1330.68), NOT strings with currency symbols\n`;
        prompt += `6. Dates must be in ISO format: "YYYY-MM-DD"\n`;
        prompt += `7. For arrays like "InvoiceLines", extract ALL visible line items with their full details\n`;
        prompt += `8. Use null ONLY for properties that genuinely cannot be found after thorough search\n`;
        prompt += `9. DO NOT invent property names - only use property names from the schema\n\n`;

        prompt += `=== OUTPUT FORMAT ===\n`;
        prompt += `Respond with ONLY a valid JSON object. No markdown code blocks, no explanations, no text before or after.\n`;
        prompt += `The JSON must match the schema structure exactly.\n\n`;

        prompt += `=== DOCUMENT INFO ===\n`;
        prompt += `File: ${file.name}\n`;
        prompt += `Document type: ${schema.name.replace(' Schema', '').replace(' (Full)', '').replace(' (Simple)', '')}\n\n`;

        prompt += `Now analyze the document image(s) and extract ALL data into the JSON schema format.`;

        console.log('[DocumentManagement] === VISION MAPPING PROMPT ===');
        console.log('[DocumentManagement] File:', file.name);
        console.log('[DocumentManagement] Schema:', schema.name);
        console.log('[DocumentManagement] Prompt length:', prompt.length);
...
If information is not found, indicate this clearly rather than making assumptions.`;
    ...
    async generateThumbnail(file: UploadedFile): Promise<string | undefined> {
        try {
            ...
            if (this.isPdfFile(file) && file.file) {
                const { getPdfJs } = await import('../../../../core/utils/pdf-worker');
     ...
     * Also fixes MIME type issues where dataUrl has 'application/octet-stream' instead of 'application/pdf'
     */
    getSanitizedPdfUrl(file: UploadedFile): SafeResourceUrl | null {
        let url = file.dataUrl || file.objectUrl;
        if (!url) return null;

        // Fix MIME type issue: some dataUrls come with 'octet-stream' which browsers can't render as PDF
        // Replace with 'application/pdf' for proper PDF rendering in iframes
     ...
     * Uses document-detected currency for monetary values
     */
    formatDisplayValue(value: any, propertyName?: string): string {
        if (value === null || value === undefined) return '';
        if (typeof value === 'boolean') return value ? 'Yes' : 'No';
        if (typeof value === 'number') {
            // Check if this is a currency/money field based on property name
            if (propertyName && this.isCurrencyField(propertyName)) {
                const currencySymbol = this.getDetectedCurrencySymbol();
                return `${currencySymbol}${value.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
            }
            // Format currency-like numbers (large numbers with cents)
    ...
    /**
     * Check if a property name indicates a currency/money field
     */
    private isCurrencyField(propertyName: string): boolean {
        const currencyKeywords = [
            'amount', 'price', 'total', 'subtotal', 'tax',
            'fee', 'cost', 'payment', 'balance', 'credit',
            'debit', 'sum', 'value', 'charge', 'discount'
        ];
    ...
    /**
     * Get the detected currency symbol from the current document
     * Looks for Currency field in mappings and returns appropriate symbol
     */
    getDetectedCurrencySymbol(): string {
        const file = this.currentPreviewFile();
        if (!file?.mappedData?.mappings) return '€'; // Default to Euro

        // Look for a Currency field in the mappings
        const currencyMapping = file.mappedData.mappings.find(m =>
            m.propertyName.toLowerCase() === 'currency' ||
            m.propertyName.toLowerCase() === 'currencycode'
        );

        if (currencyMapping?.extractedValue) {
            return this.currencyCodeToSymbol(currencyMapping.extractedValue);
        }

        // Also check parsed document for currency
        if (file.mappedData.parsedDocument?.Currency) {
            return this.currencyCodeToSymbol(file.mappedData.parsedDocument.Currency);
        }
        if (file.mappedData.parsedDocument?.CurrencyCode) {
            return this.currencyCodeToSymbol(file.mappedData.parsedDocument.CurrencyCode);
        }

        return '€'; // Default to Euro for Dutch invoices
    }

    /**
     * Convert currency code to symbol
     */
    private currencyCodeToSymbol(code: string): string {
        ...
        return symbols[upperCode] || upperCode || '€';
    }
    ...
    /**
     * Convert camelCase property name to dot-separated JSON path
     * Example: DebtorAccountID -> Debtor.Account.ID
     */
    toJsonPath(propertyName: string): string {
        if (!propertyName) return '';
        // Insert dot before each uppercase letter (except first char and consecutive uppercase like ID)
        // Split on uppercase letters while keeping them
        return propertyName
            .replace(/([A-Z]+)([A-Z][a-z])/g, '$1.$2') // Handle consecutive caps like ID followed by lowercase
